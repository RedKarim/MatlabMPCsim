...
class PedestrianCrossingSim:
    ...
    def calculate_stats(self, car_hist, controller):
        ...
        for car_data in car_hist.values():
            total_fuel += sum(car_data['fuel']) * self.dt
            avg_speed += np.mean(car_data['v'])
            if any(np.array(car_data['v']) < 0.8):
                stops += 1
...
class CasADiMPC:
    ...
    def compute_control(self, x_ego, v_ego, x_lead=None, v_lead=None, stop_x=None):
        ...
        try:
            ...
            if self.solver.stats()['success']:
                ...
                # Fallback control
                return self.fallback_control(x_ego, v_ego, x_ref, v_ref)
                
        except Exception as e:
            # Fallback in case of solver failure
            return self.fallback_control(x_ego, v_ego, x_ref if 'x_ref' in locals() else x_ego + 100, 
    ...
    def fallback_control(self, x_ego, v_ego, x_ref, v_ref):
        ...
        return max(min(acc, 2), -5)  # Bound acceleration
